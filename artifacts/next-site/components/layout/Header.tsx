...
export default function Header() {
  ...
  return (
    <header className="sticky top-0 z-50 bg-ink border-b border-border">
        ...
        <div className="flex items-center justify-between h-16 gap-4">
          ...
          <nav className="hidden md:flex items-center gap-6">
            {navLinks.map((link) => (
              <Link
                key={link.href}
                href={link.href}
                className="font-body font-medium text-sm text-paper hover:text-brand-red transition-colors"
              >
                {link.label}
              </Link>
            ))}
          ...
          <div className="flex items-center gap-3 shrink-0">
            <a
              ...
              className="sm:hidden flex items-center justify-center text-paper hover:text-brand-red transition-colors"
            >
            ...
            <a
              href="tel:+14705624020"
              className="font-body font-semibold text-sm text-paper hover:text-brand-red transition-colors hidden sm:block"
            >
              (470) 562-4020
            </a>
      ...
      {/* Mobile nav drawer */}
      {menuOpen && (
        <div className="md:hidden border-t border-border bg-ink px-4 pb-4 pt-2">
          <nav className="flex flex-col gap-3">
            {navLinks.map((link) => (
              <Link
                key={link.href}
                href={link.href}
                className="font-body font-medium text-sm text-paper hover:text-brand-red transition-colors py-1"
